...
output_predict_svm1  <- data.frame( model= character(),
                                size= character(),
                                fill= numeric(),
                                velocity=numeric(),
                                step=numeric(),
...
output_benchmark_svm1 <- data.frame(model = character(),
                               size= character(),
                               fill= numeric(),
                               velocity=numeric(),
...
for (f in 1:length(dirs)) {
  ...
  if (size == "small" & velocity >= -1.0 & fill == 0.35){
    ...
    #######TRAINING###
    df <- filter(super_last_output, velocity != v & fill.factor == l)
    ...
    for (i in 1:length(archivos)) {
      ...
      if (step %% predict_between_n_steps == 0 & step < min_step) {
        ...
        output_predict_svm1 <- rbind(output_predict_svm1, new_data_svm) 
        toc(log = TRUE, quiet = TRUE)
    ...
    output_benchmark_svm1 <- rbind(output_benchmark_svm1, time_df)
...
save(output_predict_svm1,file="S_001_006.Rda")
save(output_benchmark_svm1,file="S_bench_001_006.Rda")
...
p1 <-   output_predict_svm1 %>%
  ggplot() + 
  # ggtitle("v=1.0") + 
  geom_point(aes(step, kappa)) + 
  xlab("Steps") + 
  ylab("Kappa") + 
  theme_bw() + 
  labs(colour="Method")
